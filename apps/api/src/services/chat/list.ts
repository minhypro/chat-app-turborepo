  ...
  return async (payload: IQueryPayLoad, callback: TEventListenerCallback) => {
    ...
    try {
      ...
      const chats = searchPattern
        ? await db.all<Chat[]>('SELECT * FROM Chats WHERE is_group = TRUE AND name LIKE ? LIMIT ? OFFSET ?', [
            searchPattern,
            payload.limit,
            payload.offset,
          ])
        : await db.all<Chat[]>('SELECT * FROM Chats WHERE is_group = TRUE LIMIT ? OFFSET ?', [
            payload.limit,
            payload.offset,
          ]);
